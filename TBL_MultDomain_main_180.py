...
from myFunc import getZModes, calWeights, Screen
comm = MPI.COMM_WORLD
fScreen = open(f'./Screen_{MPI.COMM_WORLD.rank}', "w")
fScreen.write('yes')

path='/ccc/scratch/cont005/ra5138/mahfozeo/TBL_/Re_1250/Lx_750/canon/FFT_z/'
MainOutDir='/ccc/scratch/cont005/ra5138/mahfozeo/TBL_/Re_1250/Lx_750/canon/FFT_z/POD_yp_180/'
#Y+=40
...
#iy2_=[70 , 72 , 75 , 77 , 82 , 84 , 71 ,74  , 76 , 80 , 83   , 75 , 81]
...
#        print(f'Rank {str(comm.rank)}: Reading {n}/{nBlock[-1]}', end="\r", flush=True)
        fScreen.write(f'{str(n)} \n')
        if n==nBlock[0]:
            ux_temp =f['ux'][()][:,:,::step]
            uy_temp =f['uy'][()][:,:,::step]
            uz_temp =f['uz'][()][:,:,::step]
        else:
            ux_temp =np.append(ux_temp ,f['ux'][()][:,:,::step], axis=2)
            uy_temp =np.append(uy_temp ,f['uy'][()][:,:,::step], axis=2)
            uz_temp =np.append(uz_temp ,f['uz'][()][:,:,::step], axis=2)
    Screen('')
    t1=time.time()
